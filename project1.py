...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    x = feature_matrix
    y = labels
    hinge_loss = 1 - y*((np.dot(theta, x.T))+theta_0)
    hinge_loss[hinge_loss < 0] = 0 #Hinge loss will be less than 0 or 0 if it's on the decision boundary or beyond
...
def perceptron(feature_matrix, labels, T):
    ...
    current_theta_0 = 0.0
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]): #shape() means (n,m). So shape[0] means m shape[1] means m
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            average_theta = current_theta + average_theta  
            average_theta_0 = current_theta_0 + average_theta_0
            count = count+1
    return(average_theta/count, average_theta_0/count)
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        for i in get_order(x.shape[0]):
            ...
            current_theta,current_theta_0 = pegasos_single_step_update(x[i],y[i],L,eta,current_theta,current_theta_0)
    return(current_theta,current_theta_0)
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    classifier  = np.dot(theta,feature_matrix.transpose())+theta_0
...
def classifier_accuracy(
    ...
    validity = classify(val_feature_matrix,theta,theta_0)

    check_train = accuracy(trained,train_labels)
    check_val = accuracy(validity,val_labels)
    return(check_train,check_val)
